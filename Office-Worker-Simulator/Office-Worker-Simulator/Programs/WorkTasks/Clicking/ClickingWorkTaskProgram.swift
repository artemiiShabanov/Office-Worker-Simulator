...
class ClickingWorkTaskProgram: WorkTaskProgram {
    ...
    private var scene: ClickingScene!
    
    private lazy var backgroundMusic: AVAudioPlayer? = {
        guard let url = Bundle.main.url(forResource: "clicking_theme", withExtension: "wav") else {
            return nil
        }
        do {
    ...
    func render(in window: UIView) {
        ...
        scene.clickingSceneDelegate = self
...
extension ClickingWorkTaskProgram: ClickingSceneDelegate {
